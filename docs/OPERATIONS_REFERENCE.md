...
Contents: Environment and Setup · Machine State & JSON · Conflict Resolution · Learning and Memory Storage · Improvement Proposals · Scope Expansion Protocol.
...
Sessions stay Markdown: `memory/sessions/*.md` is the source of truth (with the
pinned `Status: active|interrupted|completed` parse); `.tasks/sessions.json` is
only a derived cursor — see [§ 6](#6-machine-state--json-reference).
...
## 6. Machine state & JSON reference

(origin: orchestrator "JSON machine-state conventions" + "Task state via state.sh")

Machine-readable state is JSON; human-readable knowledge/reports/plans/lessons
are Markdown; logs are JSONL. Task status is NEVER parsed from Markdown — it
comes from `tasks.json` only.

| State surface | Path | Role |
|---|---|---|
| Per-goal task state | `.tasks/<goal>/tasks.json` | **authoritative task state** — statuses, assignments, dependencies, timestamps (per-goal) |
| Agent roster | `.tasks/agents.json` | derived registry (from `agents/*.md` frontmatter), regenerated by `state.sh agents sync`; never hand-edited |
| Session cursor | `.tasks/sessions.json` | derived cursor (from `memory/sessions/*.md`), regenerated by `state.sh sessions sync`; never read by `memory-lifecycle.sh` |
| Event log | `.tasks/events.jsonl` | append-only JSONL journal; one JSON object per line; every state mutation appends one line |

All state lives under `.tasks/` — project-local, local-only, gitignored, never
installed.

### `state.sh` subcommands

`scripts/state.sh` (installed to runtime as `bin/state.sh`, bash + jq 1.7) is
the operating mechanism for task state; only it mutates `tasks.json`:

| Group | Subcommands |
|---|---|
| Mutating | `task create`, `task assign`, `task status`, `task complete`, `task fail`, `task cancel`, `task deps`, `task blocked-detect` |
| Read-only queries | `query pending`, `query agent <a>`, `query status <s>` |
| Journal | `log tail [-n N]` |
| Derivations | `agents sync`, `sessions sync` |
| Help | `--help` |

Only sanctioned transitions are allowed (7-state FSM); the Orchestrator's two
execution flips run through `state.sh` into `tasks.json`:
`task status <NN> in_progress` at dispatch; `task status <NN> completed` only
with verified-completion evidence. Implementers never self-flip their own task
state.

### Task statuses (7-state enum)

`pending | assigned | in_progress | blocked | completed | failed | cancelled`

Terminal: `completed`, `failed`, `cancelled`. Timestamps ISO-8601 UTC;
`started_at` on entering `in_progress`, `completed_at` on entering `completed`,
`updated_at` on every mutation.

### Sessions coexistence

`memory/sessions/*.md` remains the source of truth for sessions (format and
`Status: active | interrupted | completed` contract: `memory/sessions/README.md`);
`.tasks/sessions.json` is only the derived cursor for machine queries. Keep
writing/updating the Markdown session file exactly as today.

### events.jsonl logging

Append-only; chronological order = file order. Every event has the envelope
`ts` (ISO-8601 UTC), `event`, `by` (`orchestrator` | `tool` | agent name);
vocabulary includes `task.created`, `task.assigned`, `agent.dispatched`,
`task.status_changed`, `task.dep_added`, `session.started`/`session.interrupted`/
`session.completed`, `report.written`. Every line must parse as JSON (see
`state.sh log tail`).

---

*End of Operations Reference. Summary of extraction origins for maintainers: §1 ← orchestrator §5 (env facts); §2 ← orchestrator §21; §3 ← orchestrator §26 (storage rules canonical in orchestrator "Memory vs Task State"); §4 ← orchestrator §27; §5 ← orchestrator §29; §6 ← orchestrator "JSON machine-state conventions" / "Task state via state.sh" (state-json-migration).*